...
{
    public function testGoogleAndPasswordHashesMatchForTheSameEmail(): void
    {
        ...
        $this->assertSame(md5('ada@example.com'), $google->hash());
        $this->assertSame(md5('ada@example.com'), $password->hash());
        $this->assertSame($google->hash(), $password->hash());
        $this->assertSame(EmailKey::fromEmail('Ada@Example.COM')->hash(), $google->hash());
        $this->assertSame(32, strlen($google->hash()));
        $this->assertSame(1, preg_match('/^[a-f0-9]{32}$/', $google->hash()));
        $this->assertNotSame(md5('google|ada@example.com'), $google->hash());
        $this->assertNotSame(md5('password|ada@example.com'), $password->hash());
    }
    ...
    public function testPasswordUsernameDoesNotRequireAnEmail(): void
    {
        ...
        $this->assertSame(md5('lifter.one'), $key->hash());
        $this->assertNotSame(md5('password|lifter.one'), $key->hash());
    }
}
